 ...
 * Image img = ImageUtil.carregarImagem("/icons/logo.png");
 * 
 * // Carregar como ícone
 * Icon icon = ImageUtil.carregarIcone("/icons/botao.png");
 * 
 * // Carregar e redimensionar
 * Image imgRedim = ImageUtil.carregarImagem("/icons/logo.png", 32, 32);
 * 
 * // Redimensionar imagem existente
 * Image imgPequena = ImageUtil.redimensionar(img, 16, 16);
 * }</pre>
 * 
 * @author alefi
 */
public final class ImageUtil {
     ...
     */
    public static Image carregarImagem(String caminhoRecurso) {
        ...
        try (InputStream is = Thread.currentThread().getContextClassLoader().getResourceAsStream(caminho)) {
            if (is == null) {
                System.err.println("ImageUtil: Imagem não encontrada no classpath: " + caminho);
                return null;
            }
            return ImageIO.read(is);
        } catch (IOException e) {
            System.err.println("ImageUtil: Erro ao carregar imagem: " + e.getMessage());
            return null;
        }
    }
     ...
     */
    public static Image carregarImagem(String caminhoRecurso, int largura, int altura) {
        Image img = carregarImagem(caminhoRecurso);
    ...
    public static Icon carregarIcone(String caminhoRecurso, int largura, int altura) {
        Image img = carregarImagem(caminhoRecurso, largura, altura);
        if (img == null) return null;
        return new ImageIcon(img);
    }
    ...
    public static Image carregarDeUrl(String url) {
        try {
            ...
            System.err.println("ImageUtil: Erro ao carregar imagem da URL: " + e.getMessage());
            return null;
        }
    }
     ...
     */
    public static BufferedImage toBufferedImage(Image imagem) {
